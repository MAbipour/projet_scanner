 ...
 *
 *  Created on: mai 2022
 *      Author: Code basé sur le TP4 du cours "Système embarqué et robotique" 
...
uint16_t extract_line_width(uint8_t *buffer)
{
	...
	do{
		...
		if (i < (Y_IMAGE_SIZE - WIDTH_SLOPE) && begin)
		{
		    ...
		    {
		        if(buffer[i] > mean + CORR_COEFF && buffer[i - WIDTH_SLOPE] 
		           < mean - CORR_COEFF)
		        {
		            end = i;
		            stop = 1;
...
static THD_FUNCTION(CaptureImage, arg) 
{
    ...
    po8030_advanced_config(FORMAT_RGB565, X_IMAGE_SIZE/2, 0, 2, 
    					   Y_IMAGE_SIZE, SUBSAMPLING_X1, SUBSAMPLING_X1);
...
static THD_FUNCTION(ProcessImage, arg) 
{
    ...
    while(1)
    {
		...
		if(cam_mode == MODE_CAMERA_ON)
		{
			...
			if (MIN_LINE_WIDTH < lineWidth && lineWidth 
				< THRESH_LINE_WIDTH_1)
			{
			...
			if (lineWidth > THRESH_LINE_WIDTH_1 && lineWidth < 
				THRESH_LINE_WIDTH_2)
			{
